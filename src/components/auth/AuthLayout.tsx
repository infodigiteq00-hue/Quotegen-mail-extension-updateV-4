    ...
    <div className="min-h-screen bg-gradient-mesh flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center mb-6 sm:mb-8">
          ...
          <h1 className="text-xl sm:text-2xl font-semibold text-center">{title}</h1>
          {subtitle && <p className="text-sm text-muted-foreground text-center mt-1">{subtitle}</p>}
        </div>
        <div className="rounded-xl border bg-card/80 backdrop-blur p-4 sm:p-6 shadow-lg">{children}</div>
      </div>
    </div>
  );
}
